...
async function runIngestion() {
  ...
  try {
    ...
    console.log("[DB] Logging run success to scraper_runs...");
    ...
    console.error(`\n[ERROR] Ingestion run failed: ${errMsg}`);

    // Log the failed run to scraper_runs table
    console.log("[DB] Logging run failure to scraper_runs...");
    try {
      await db.insert(scraperRuns).values({
        id: randomUUID(),
        collector_id: collectorId,
